...
async function run() {
  console.log("=== Búsqueda de Usuarios en Supabase Auth ===");
  try {
    const authUsers: any[] = await prisma.$queryRaw`SELECT id, email, raw_user_meta_data, created_at, last_sign_in_at FROM auth.users`;
    ...
    for (const au of authUsers) {
      ...
      let reason = [];
      if (isTestByEmail) reason.push('Test email pattern');
      if (!publicUser) reason.push('Orphan in public.User');
