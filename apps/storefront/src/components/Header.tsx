    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm w-full pt-[env(safe-area-inset-top)]">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 py-2 lg:py-3 flex flex-col gap-2 lg:gap-3">
        ...
        <nav className="hidden lg:flex items-center gap-6 pl-0 pb-1">
          ...
          <Link href={user ? `/${country}/vendedor/onboarding` : `/${country}/auth-v2/signup?intent=seller&next=${encodeURIComponent(`/${country}/vendedor/onboarding`)}`} className={isActive(`/${country}/vendedor`) ? activeClass : inactiveClass}>{dict.header.sell}</Link>
        ...
        <div className="fixed inset-0 z-[100] bg-white flex flex-col h-[100dvh] overflow-hidden lg:hidden animate-in slide-in-from-left-4 duration-200">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-8 flex flex-col gap-8">
             ...
             <nav className="flex flex-col gap-5">
                ...
                <Link href={user ? `/${country}/vendedor/onboarding` : `/${country}/auth-v2/signup?intent=seller&next=${encodeURIComponent(`/${country}/vendedor/onboarding`)}`} onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-bold text-gray-700 flex items-center gap-3">
                  🏪 {dict.header.sell}
                </Link>
