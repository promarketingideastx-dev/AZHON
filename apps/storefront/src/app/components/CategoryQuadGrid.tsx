        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col h-full">
            <div className="flex items-end justify-between mb-6">
              <h2 className="text-2xl font-black text-gray-900 tracking-tight">{dict?.home?.discover_more || 'Descubre Más'}</h2>
            </div>
                  ...
                  <Link href={`/${country}/producto/${product.id}`} key={idx} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col border border-gray-100">
                    <div className="aspect-square bg-white flex items-center justify-center p-4">
                      {product.Media?.[0]?.url ? (
                        ...
                        <span className="text-[10px] text-neutral opacity-50">{dict?.home?.no_photo || 'Sin foto'}</span>
                      )}
                    </div>
                    <div className="p-3 border-t border-gray-50 flex-1 flex flex-col">
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1 line-clamp-1">{product.Category?.name || dict?.home?.category || 'Categoría'}</span>
          ...
          <div className="lg:col-span-4 flex flex-col h-full">
            <div className="flex items-end justify-between mb-6">
              <h2 className="text-2xl font-black text-gray-900 tracking-tight">{dict?.home?.best_sellers || 'Más Vendidos'}</h2>
            </div>
          ...
          <div className="lg:col-span-3 flex flex-col h-full">
             <div className="flex items-end justify-between mb-6">
              <h2 className="text-2xl font-black text-gray-900 tracking-tight">{dict?.home?.just_for_you || 'Solo para Ti'}</h2>
            </div>
            <Link href={`/${country}/producto/${promoProduct.id}`} className="bg-[#1A1816] rounded-2xl p-6 flex-1 relative overflow-hidden group block shadow-lg border border-gray-800">
              ...
              <div className="relative z-20 h-full flex flex-col justify-end">
                <span className="text-xs font-bold text-white/70 mb-2 uppercase tracking-wider">{promoProduct.Category?.name || dict?.home?.recommended || 'Recomendado'}</span>
                <h3 className="text-lg font-black text-white leading-tight mb-2 line-clamp-3">{promoProduct.title}</h3>
                <span className="inline-block bg-primary text-white text-xs font-bold px-4 py-2 rounded-xl mt-4 self-start">{dict?.home?.view_offer || 'Ver Oferta'}</span>
              </div>
            </Link>
          </div>
