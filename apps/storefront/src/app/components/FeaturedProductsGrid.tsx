  ...
  return (
    <section className="max-w-[1440px] mx-auto px-4 sm:px-6 py-12 border-t border-gray-100 mt-12 mb-12">
      <div className="flex items-end justify-between mb-8">
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">{dict?.home?.product_feed || 'Feed de Productos'}</h2>
          <p className="text-gray-600 text-sm mt-1">{dict?.home?.browse_catalog || 'Navega el catálogo completo'}</p>
        </div>
      </div>
      ...
      <div className="flex justify-center mt-10">
         <button className="bg-white border border-gray-300 text-gray-800 font-bold px-8 py-3 rounded-full shadow-sm hover:bg-gray-50 transition-colors text-sm">
           {dict?.home?.load_more || 'Cargar más resultados'}
         </button>
      </div>
    </section>
  );
