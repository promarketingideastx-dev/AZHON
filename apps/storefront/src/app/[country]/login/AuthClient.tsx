...
export default function AuthClient({ dict, errorKey, msgKey, intent, defaultEmail }: { dict: any, errorKey?: string, msgKey?: string, intent?: string, defaultEmail?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    // We let Server Actions handle the actual submission natively for forms
    // But we could toggle loading state
    setLoading(true);
  };
  ...
  return (
    <div className="bg-background p-10 rounded-2xl shadow-sm border border-gray-100 max-w-sm w-full">
      ...
      <h1 className="text-2xl font-bold mb-2 text-center text-secondary tracking-tight">
        {view === 'login' ? (t.title_login || 'Bienvenido a AZHON') :
         view === 'signup' ? (t.title_signup || 'Crear una cuenta') :
          ...
          <div className="flex flex-col gap-3 mt-4">
            {view === 'login' && (
              <>
                ...
                <button formAction={login} className="w-full bg-secondary text-white rounded-full py-3.5 font-bold hover:bg-black transition-colors shadow-sm disabled:opacity-50">
                ...
                <button formAction={signInWithGoogle} className="w-full bg-white text-secondary border border-gray-200 rounded-full py-3.5 font-bold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
            ...
            {view === 'signup' && (
              <>
                <button formAction={signup} className="w-full bg-primary text-white rounded-full py-3.5 font-bold hover:bg-orange-600 transition-colors shadow-sm disabled:opacity-50">
